...
double getdouble(){
  ...
  int i = 0;
  char character = 'c';
  char **endpointer= NULL;  // has the address of the last character that was converted into a double
  ...
  /*convert to a double*/
  number = strtod(buffer, endpointer);
...
/*
  ...
  The number may consist of an optional sign, a string of digits with an optional decimal character, 
  and an optional e or E followed by a optionally signed exponent. 
  Conversion stops when the first unrecognized character is reached.  
  The argument endptr is a pointer to a pointer. 
  The address of the character that stopped the scan is stored in the pointer that endptr points to.  
  On success the converted number is returned. 
  If no conversion can be made, zero is returned. 
  If the value is out of range of the type double, then HUGE_VAL is returned with the appropriate sign and ERANGE is stored in the variable errno. 
  ...
  See this webpage "How strtod() Works (and Sometimes Doesn't" for even more details: 
