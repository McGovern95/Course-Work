	...
	if(isset($_POST["submit"])){
	  ...
	  $user_id = "";
		...
		if(empty($_POST["pword"])){
			...
			$query_string = "SELECT * FROM player WHERE loginid = '".$_POST["uname"]."' and password = '".$_POST["pword"]."';";
			...
			if($query_result->num_rows == 1){
				...
				if($r["accountstatus"] == "active"){
					...
					$_SESSION["user_id"] = $r["playerid"];
					header("Location: player_welcome.php"); 
					exit;
				}
				else if($r["accountstatus"] == "pending"){
					$login_result_message = "Account not active yet, waiting for Manager to activate";
				}
				else if($r["accountstatus"] == "rejected"){
					$login_result_message = "Account rejected by Manager, access denied";
				}
			}
			else{
		...
		header("Location: player_reset_password.php"); 
		exit;
	}
}
?>
