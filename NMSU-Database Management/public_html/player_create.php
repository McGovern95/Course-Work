...
$login_id_msg = $full_name_msg = $password_msg = $address_msg = $email_msg = $phone_number_msg = $play_pos_msg = $request_msg = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {			// create manager account request
	...
	$play_pos = validate_input($_POST["play_pos"]);
	...
	if(empty($_POST['play_pos'])){
		$play_pos_msg = "required field!";
		$valid_form = false;
	}
	...
	if($valid_form){
		...
		$query_string = "select playerid from player where loginid = '".$login_id."' "
			."union select managerid from manager where loginid = '".$login_id."';";
		...
		if($query_result->num_rows > 0){		// this login id already taken!
			...
			$query_string = "insert into player(loginid, name, password, birthday, address, email, phonenumber, playpos, accountstatus)"
				." value('".$login_id."','".$full_name."','".$login_password."',STR_TO_DATE('".$birthday."', '%m-%d-%Y'),'".$address
				."','".$email."','".$phone_number."','".$play_pos."','pending')";

			if ($conn->query($query_string) === TRUE) {
				$request_msg = "Account requested, redirecting to login page...";
				$conn->commit();
...
<body>
		<form method=post action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
		  <table align="center">
			    ...
			    <tr>
			    	<td style="text-align: right">Player Position</td>
			        <td><input required type="text" name="play_pos" value="<?php echo isset($_POST["play_pos"])? validate_input($_POST["play_pos"]) : ''; ?>"></td>
					<td><?php echo $play_pos_msg?></td>
			   </tr>
